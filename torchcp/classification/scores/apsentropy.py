...
class APSENTROPY(BaseScore):
    ...
    def __init__(self, temperature=1):
        ...
        if temperature <= 0:
            raise ValueError("The parameter 'temperature' must be a positive value.")
        super(APSENTROPY, self).__init__()
    ...
    def __call__(self, logits, label=None):
        ...
        # Reweight the logits based on temperature and entropy
        probs = torch.softmax(logits, dim=-1)
        entropy = -torch.sum(probs * torch.log(probs), dim=-1, keepdim=True)
        logits = logits / (1 + entropy) / self.temperature
